...
No sentence-transformers — uses Gemini API for embeddings.
Runs comfortably within 512MB RAM on Render free tier.
...
# ══════════════════════════════════════════════════════════════════════════════
# Gemini Embeddings  (replaces sentence-transformers — zero RAM cost)
# ══════════════════════════════════════════════════════════════════════════════

async def get_gemini_embedding(text: str) -> list[float]:
    """Call Gemini text-embedding-004 API and return 768-dim vector."""
    payload = {
...
async def retrieve_from_pinecone(query_text: str, top_k: int = 20) -> list[dict]:
    """Embed query with Gemini → search Pinecone → return candidates."""
...
async def rerank_with_gemini(query: str, candidates: list[dict]) -> list[dict]:
    """Use Gemini to pick best 5-10 assessments with balanced skill coverage."""
...
2. BALANCE technical and behavioral assessments:
   - Technical skills needed → include Knowledge & Skills (K) type.
